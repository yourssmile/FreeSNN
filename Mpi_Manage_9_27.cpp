...
#define RECV_EVENT_LEN 50000//MPI接收脉冲缓冲区
...
class Mpi_Manage
{
                ...
                for (int left_pop_point = left_pop; left_pop_point < left_pop+pop_map[left_pop]; left_pop_point++)
                {
                    for (int right_pop_point = right_pop; right_pop_point < right_pop+pop_map[right_pop]; right_pop_point++)
                    {
                        ...
                        if(rate>=random_num)//按照rate的大小进行概率连接
                        {
                            ...
                            {
                                double random_num_stdp=double(random(1000))/double(1000);//生成一个小数点后三位的小数
                                if(0.6>random_num_stdp && right_pop_point%4==0)
                                {
                                    STDP_adj_table[left_pop_point].push_back(right_pop_point);
                                    STDP_w_table[left_pop_point].push_back(weight);  
                                }
                                else if(0.9>random_num_stdp)
                                {
                                    STDP_adj_table[left_pop_point].push_back(right_pop_point);
                                    STDP_w_table[left_pop_point].push_back(weight);   
                                }
    ...
    void read_Gid_to_rank(string G_t_r_read_txt)
    {
        ...
        cout<<"****************************************************"<<endl;
        cout<<"G_t_r_read_txt is : "<<G_t_r_read_txt<<endl;
        cout<<"****************************************************"<<endl;
        ...
        void simulate(int sim_time)//开始模拟，输入模拟时间
        {
            ...
            double mini_mini_time=0;
            double total_mini_mini_time=0;
            double STDP_time=0;
            ...
            for (int loop_step = 0; loop_step < loop_steps; loop_step++)
            {
                ...
                if(loop_debug)
                cout<<"rank "<<MPI_rank<<" loop_step : "<<loop_step<<" get_event()"<<endl;

                get_event();
                //gettimeofday(&get_handle_event_t2, nullptr);
                if(loop_debug)
                cout<<"rank "<<MPI_rank<<" loop_step : "<<loop_step<<" handle_event()"<<endl;
                ...
                if(loop_debug)
                cout<<"rank "<<MPI_rank<<" loop_step : "<<loop_step<<" update_STDP()"<<endl;
                ...
                if(loop_debug)
                cout<<"rank "<<MPI_rank<<" loop_step : "<<loop_step<<" sim_update()"<<endl;
                ...
                if(loop_debug)
                cout<<"rank "<<MPI_rank<<" loop_step : "<<loop_step<<" send_event()"<<endl;
                send_event(merge_adj_table,&send_time,&mini_mini_time);//分发事件
                total_mini_mini_time+=mini_mini_time;
                ...
                if(loop_debug)
                cout<<"rank "<<MPI_rank<<" loop_step : "<<loop_step<<" Clear_event()"<<endl;
                Clear_event_set();//清除事件
                if(loop_debug)
                cout<<"rank "<<MPI_rank<<" loop_step : "<<loop_step<<" Clear_event_finished()"<<endl;
            ...
            ofstream ofs;
            vector<string> adder_of_resoult_v={"resoult0.txt","resoult1.txt","resoult2.txt","resoult3.txt","resoult4.txt","resoult5.txt","resoult6.txt","resoult7.txt","resoult8.txt"};

            string adder_of_resoult=adder_of_resoult_v[MPI_rank];
            ofs.open(adder_of_resoult,ios::out); 
            ofs<<"TIME:::!rank : "<<MPI_rank<<" get_handle_time : "<<get_handle_time<<" handle_STDP_time : "<<handle_STDP_time<<" STDP_time : "<<STDP_time<<" update_time : "<<update_time<<" send_event_time : "<<send_event_time<<" mini_send_event_time : "<<mini_send_time<<"mini_mini_time:"<<total_mini_mini_time<<endl;
            ofs.close();
        ...
        void send_event(vector<vector<int>> merge_adj_table,double * send_time,double *mini_mini_time)
        {  
            ...
            send_event_to_rank(mini_mini_time);//把send_to_rank数据分发到不同rank；
        ...
        void send_event_to_rank(double * mini_mini_time)
        {
            double time ;
            double mini_mini_send=0;
                ...
                MPI_Win_fence(0, r_f_win[com_rank]);
                    ...
                    if(send_to_rank[rank_num][0]>0 && send_out_debug)
                    {
                     cout<<"send_to_rank["<<rank_num<<"][0]= "<<send_to_rank[rank_num][0]<<" rank is "<<MPI_rank<<" ["<<rank_num<<"][1] "<<send_to_rank[rank_num][1]<<" ["<<rank_num<<"][2] "<<send_to_rank[rank_num][2]<<endl;
                            ...
                            {
                            // if((send_to_rank[rank_num][0]+2)>PUT_BUF_LEN)
                            // {
                                //cout<<"send_to_rank[rank_num][0]+2 is : "<<send_to_rank[rank_num][0]+2<<endl;
                                //cout<<"PUT_BUF_LEN is : "<<PUT_BUF_LEN<<endl;
                            //}
                            time = MPI_Wtime();
                            //cout<<"start put "<<endl;
