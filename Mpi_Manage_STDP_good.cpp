    ...
    public:
        ...
        void send_Gid_to_rank()
        {
            ...
            if(MPI_rank!=0)
            {
                ...
                vector<int> rec_v=int_set_to_vector(recv_Gid_to_rank,neuron_rec_num);
                map<int,int> rec_map;
        ...
        void send_adj_table()//把表从0号进程传送去其他进程，进行连接表同步
        {
            ...
            if(MPI_rank!=0)
            {
                ...
                vector<int> rec_v=int_set_to_vector(recv_adj_table,rec_count);
        ...
        void send_weight_table()//把表从一号进程传送去其他进程，进行连接表同步
        {
            ...
            if(MPI_rank!=0)
            {
                ...
                cout<<"this is rank "<<MPI_rank<<" rec_count is "<<rec_count<<endl;
                vector<double> rec_v=int_set_to_vector(recv_weight_table,rec_count);
        ...
        void send_stdp_adj_table()//把表从一号进程传送去其他进程，进行连接表同步
        {
            ...
            if(MPI_rank!=0)
            {
                ...
                cout<<"this is rank "<<MPI_rank<<" rec_count is "<<rec_count<<endl;
                vector<int> rec_v=int_set_to_vector(recv_stdp_adj_table,rec_count);
        ...
        void send_stdp_w_table()//把表从一号进程传送去其他进程，进行连接表同步
        {
            ...
            if(MPI_rank!=0)
            {
                ...
                cout<<"this is rank "<<MPI_rank<<" rec_count is "<<rec_count<<endl;
                vector<double> rec_v=int_set_to_vector(recv_stdp_w_table,rec_count);
                STDP_w_table=one_v_to_2_v(rec_v);
            }

        }

        void record_Gid_to_rank()
        {
            vector<int> send_Gid_to_rank;
            send_Gid_to_rank.push_back(-1);
            map<int,int>::iterator it;
            ...
            ofstream ofs;
            ofs.open("Gid_to_rank.txt",ios::out);
        ...
        void record_adjtable()//记录adjtable到txt
        {
            ofstream ofs;
            ofs.open("adj_table.txt",ios::out);
        ...
        void record_weight_table()//记录adjtable到txt
        {
            ofstream ofs;
            ofs.open("weight_table.txt",ios::out);
        ...
        void record_STDP_adj_table()//记录adjtable到txt
        {
            ofstream ofs;
            ofs.open("STDP_adj_table.txt",ios::out);
        ...
        void record_STDP_w_table()//记录adjtable到txt
        {
            ofstream ofs;
            ofs.open("STDP_w_table.txt",ios::out);
        ...
        void read_Gid_to_rank()
        {
            int i,datalen=0;
            int num[9000];
            ifstream file("Gid_to_rank.txt");
            while( ! file.eof() ){
            file>>num[datalen++];
            ...
            vector<int> rec_v=int_set_to_vector(num,neuron_rec_num);
            map<int,int> rec_map;
            for (size_t i = 1; i < neuron_rec_num; i+=2)
            {
                rec_map[rec_v[i]]=rec_v[i+1];
        ...
        void read_adj_table()
        {
            int i,datalen=0;
            int num[30000];
            ifstream file("adj_table.txt",ios::in);
            while( ! file.eof() ){
            file>>num[datalen++];
            //cout<<"len is :"<<datalen<<" num is : "<<num[datalen-1]<<endl;
            }
            ...
            for (size_t i = 0; i < datalen-1; i++)
            {
                ...
                temp_adj_table[line_index].push_back(num[i]);              
            }
        ...
        void read_weight_table()
        {
            ...
            int i,datalen=0;
            double num[30000];
            ifstream file("weight_table.txt",ios::in);
            while( ! file.eof() ){
            file>>num[datalen++];
            //cout<<"len is :"<<datalen<<" num is : "<<num[datalen-1]<<endl;
            }
            ...
            for (size_t i = 0; i < datalen-1; i++)
            {
                ...
                temp_weight_table[line_index].push_back(num[i]);              
            }  
            cout<<"read_end"<<endl;
            weight_table=temp_weight_table;       
        }

        void read_STDP_adj_table()
        {
            int i,datalen=0;
            double num[30000];
            ifstream file("STDP_adj_table.txt",ios::in);
            while( ! file.eof() ){
            file>>num[datalen++];
            //cout<<"len is :"<<datalen<<" num is : "<<num[datalen-1]<<endl;
            }
            ...
            for (size_t i = 0; i < datalen-1; i++)
            {
                ...
                temp_STDP_adj_table[line_index].push_back(num[i]);              
            }
            STDP_adj_table=temp_STDP_adj_table;          
        }

        void read_STDP_w_table()
        {
            cout<<"read_in"<<endl;
            int i,datalen=0;
            double num[30000];
            ifstream file("STDP_w_table.txt",ios::in);
            while( ! file.eof() ){
            file>>num[datalen++];
            //cout<<"len is :"<<datalen<<" num is : "<<num[datalen-1]<<endl;
            }
            ...
            for (size_t i = 0; i < datalen-1; i++)
            {
                ...
                temp_STDP_w_table[line_index].push_back(num[i]);              
            }
            cout<<"read_end"<<endl;
        ...
        void create_STDP()
        {
            ...
            for (size_t i = 0; i < STDP_adj_table.size(); i++)
            {
                ...
                {
                    if(search(neuron_Gid,STDP_adj_table[i][j]))
                    {
                        STDP stdp_;
        ...
        void simulate(int sim_time)//开始模拟，输入模拟时间
        {
            ...
            for (int loop_step = 0; loop_step < loop_steps; loop_step++)
            {
                ...
                send_event(merge_adj_table);//分发事件
                Clear_event_set();//清除事件
            ...
            cout<<"TIME:::!rank : "<<MPI_rank<<" STDP_time : "<<STDP_time<<" update_time : "<<update_time<<endl;
        }
        ...
        void handle_event(int loop_step)
        {
        ...
        void STDP_syn_update(int i,int loop_step)
        {
            ...
            if(STDP_adj_table[source_n].size()>1)//如果这个source神经元后面有STDP连接
            {
                for (size_t STDP_syn_point = 1; STDP_syn_point < STDP_adj_table[source_n].size(); STDP_syn_point++)
                {
                    ...
                    for (int stdp_index = 0; stdp_index < STDP_set.size(); stdp_index++)
                    {
        ...
        void send_event(vector<vector<int>> merge_adj_table )
        {  
...
//MPI 测试
    ...
    //  pop1=ma.create(1,1);
    //  pop2=ma.create(1,1);
    // ma.STDP_full_connect(pop1,pop2,10);
    ...
    //     ma.Poisson_full_connect(5000,100,0,1);
    //     ma.Poisson_full_connect(3000,500,1,2);


    // //possion
    // // cout<<"STDP weight before:_____________________________________________ ";
    // cout<<"this is rank: "<<ma.MPI_rank<<" STDP_set.size() is "<<ma.STDP_set.size()<<" neuron_set size is: "<<ma.neuron_set.size()<<endl;
    // for (size_t i = 0; i < ma.STDP_set.size(); i++)
    // {
    //     cout<<"STDP weight before:_____________________________________________ ";
    //     cout<<ma.STDP_set[i].weight<<" ";
    // }
    // cout<<endl;
    
    // ma.simulate(300);

    // // cout<<"STDP weight: _____________________________________________ ";
    // for (size_t i = 0; i < ma.STDP_set.size(); i++)
    // {
    //     cout<<"STDP weight: _____________________________________________ ";
    //     cout<<ma.STDP_set[i].weight<<" ";
    ...
    // }
    // cout<<endl;
    //     cout<<"connect table start"<<endl;
    // for (size_t i = 0; i < ma.STDP_adj_table.size(); i++)
    // {
    //     for (size_t j = 0; j < ma.STDP_adj_table[i].size(); j++)
    //     {
    //         cout<<ma.STDP_adj_table[i][j]<<" ";
    //     }
    //     cout<<endl;
        
    // }
    // cout<<"connect table end"<<endl;
    ...
    // ma.send_Gid_to_rank();
    // ma.send_adj_table();
    // ma.send_weight_table();
    // ma.create_this_rank_neuron();
    // if(ma.MPI_rank==1)
    // {
    //     cout<<"print,neuron_set:"<<ma.neuron_set.size()<<endl;
    //     for (int i = 0; i < ma.neuron_set.size(); ++i)
    //     {
    //         cout<<ma.neuron_set[i].global_num<<" ";
    //     }
    //     cout<<endl;
    //     print_map(ma.Gid_to_rank);
    // }
    // if(ma.MPI_rank==1){
    //     cout<<"print_recv_table:"<<endl;
    //     for (int i = 0; i < 200; ++i)
    //     {
    //         cout<<ma.recv_table[i]<<" ";
    //         if(i%10==0)
    //             cout<<endl;
    //     }
    //     cout<<"print_recv_table end"<<endl;
    // }
    // ma.create_this_rank_neuron();//创建当前进程上的神经元
    // //ma.Poisson_full_connect(50,pop1);//打开poisson激励，多进程,需修改
    // ma.simulate(100);//模拟，多进程
    ...
    // pop1=ma.create(1,3);
    // pop2=ma.create(1,3);
    // ma.full_connect(pop1,pop2,100);
    // ma.Poisson_full_connect(50,pop1);
    // ma.simulate(400);
    ...
    // int pop1;
    // pop1=ma.create(1,3);
    // ma.Poisson_full_connect(10,pop1);
    // ma.print_vm_switch=1;//打开输出Vm开关
    // ma.print_Vm_G_id=0;//输出0号神经元的Vm
    // ma.simulate(200);
    ...
    // pop1=ma.create(1,2);
    // pop2=ma.create(1,4);
    // //ma.full_connect(pop1,pop2,1);
    // ma.random_connect(pop1,pop2,1,0.5);
    // ma.simulate(100);
    // // for (size_t i = 0; i < ma.neuron_set.size(); i++)
    // // {
    // //     int global_num= ma.neuron_set[i].Get_globel_num();
    // //     cout<<" global num is "<<global_num<<endl;
    // // }
    // cout<<"connect table start"<<endl;
    // for (size_t i = 0; i < ma.adjacency_table.size(); i++)
    // {
    //     for (size_t j = 0; j < ma.adjacency_table[i].size(); j++)
    //     {
    //         cout<<ma.adjacency_table[i][j]<<" ";
    //     }
    //     cout<<endl;
